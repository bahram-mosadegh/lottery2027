...
function change_status(id, status){
  window.location.replace("/change-account-status/"+id+"/"+status);
}

function delete_account(id, confirm = 0, username = ''){
  if(confirm == 0){
      $('#delete-account').modal('show');
      document.getElementById('delete-account-id').innerHTML = username;
      document.getElementById('delete-account-button').innerHTML = '<button onclick="delete_account('+id+', 1);" type="button" class="btn bg-gradient-primary">Yes, Delete!</button>';
  }else{
      window.location.replace("/delete-account/"+id);
  }
}

function user_status(id, status){
  window.location.replace("/change-user-status/"+id+"/"+status);
}

function delete_user(id, confirm = 0){
  if(confirm == 0){
      $('#delete-user').modal('show');
      document.getElementById('delete-user-id').innerHTML = id;
      document.getElementById('delete-user-button').innerHTML = '<button onclick="delete_user('+id+', 1);" type="button" class="btn bg-gradient-primary">Yes, Delete!</button>';
  }else{
      window.location.replace("/delete-user/"+id);
  }
}
...
function final_phone_check(str) {
...
function biometric_enrollment_city(id, status){
  window.location.replace("/change_biometric_enrollment_city_status/"+id+"/"+status);
}